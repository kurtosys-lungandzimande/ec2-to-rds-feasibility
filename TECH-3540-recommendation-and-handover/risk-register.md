...
| Second RI potentially unmatched | Medium | Medium | Hermann noted two RIs exist for Windows with SQL Server Enterprise, but only one instance bills under that licensed code. ew2p-mssql-02 is on a Compute Savings Plan instead. Whether the second RI matches elsewhere or is idle is unclear. Check via console — an idle RI is wasted spend. | Platform Engineering |
